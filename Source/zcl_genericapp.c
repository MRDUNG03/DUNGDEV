...
void zclGenericApp_Init( byte task_id )
{
  ...
  SHT30_Init();
  osal_start_timerEx(zclGenericApp_TaskID , READ_SENSOR_EVENT, 5000);
...
uint16 zclGenericApp_event_loop( uint8 task_id, uint16 events )
{
  ...
  if (events & READ_SENSOR_EVENT) {
    float temp, humi;
    if (sht30_read_temp_humi(&temp, &humi) == 0) {
      
    }
    osal_start_timerEx(zclGenericApp_TaskID , READ_SENSOR_EVENT, 5000);  //Read sensor each 5s
    return (events ^ READ_SENSOR_EVENT);
  }
